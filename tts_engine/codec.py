...
class SNACCodec:
    ...
    @track_time("SNAC.decode_windows_batch")
    def decode_windows_batch(self, windows: List[List[int]]) -> List[bytes]:
        """
        Decode multiple sliding windows in a single batched GPU call.

        Concatenates all windows into one set of code tensors, runs a single
        SNAC decode, then splits the output back per window. This amortises
        CUDA kernel launch overhead and is ~2-4x faster than decoding one
        window at a time for batches of 4+.

        Args:
            windows: List of windows, each a flat list of int codes (length
                     must be a multiple of 7).

        Returns:
            List of PCM16 byte strings, one per input window.
        """
        if not windows:
            return []
        ...
        if len(windows) == 1:
            return [self.decode_window(windows[0])]

        # Compute frame counts per window and filter invalid ones
        frame_counts = []
        valid_indices = []
        all_frames = []
        ...
        for i, window in enumerate(windows):
            ...
            all_frames.extend(window[: F * 7])

        if not all_frames:
            return [b""] * len(windows)

        total_frames = sum(frame_counts)

        # Build concatenated code tensors
        t = torch.tensor(all_frames, dtype=torch.int32, device=self.device)
        t = t.view(total_frames, 7)
        ...
        # Convert entire result to numpy once (single PCIe transfer)
        x = audio.detach().float().cpu().numpy().reshape(-1)

        # Split back into per-window results
        # Each frame produces (4096 - 2048) = 2048 samples in the hop region
        samples_per_frame = 2048
        results = [b""] * len(windows)
        offset = 0

        for i in valid_indices:
            F = frame_counts[i]
            n_samples = F * samples_per_frame
            chunk = x[offset: offset + n_samples]
            offset += n_samples
            pcm16 = (np.clip(chunk, -1.0, 1.0) * 32767.0).astype(np.int16)
            results[i] = pcm16.tobytes()
